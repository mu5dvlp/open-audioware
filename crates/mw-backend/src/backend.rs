...
pub trait Backend {
    ...
    /// 出力レイテンシの実測値を、1オープンにつき1回だけログへ出す。
    ///
    /// **ゲームスレッドから呼ぶこと。** `mw_log!` はアロケーションとロックを伴うため、
    /// 音声コールバック経路(音声スレッド)からは呼んではならない。ログの配線は
    /// バックエンド実装ごとに必要なので既定実装は設けず、将来の oboe / RemoteIO 実装が
    /// 追加時にログを失うことへ気付けるよう、必須メソッドとしている。
    fn log_output_latency_once(&self);

    /// 出力コールバックのアンダーラン(の疑い)を新たに検知していれば、その分だけログへ出す。
    ///
    /// **ゲームスレッドから、FFI 呼び出しの合間に日和見的に呼ぶこと。** `mw_log!` は
    /// アロケーションとロックを伴うため、音声コールバック経路(音声スレッド)からは
    /// 呼んではならない。ログの配線はバックエンド実装ごとに必要なので既定実装は設けず、
    /// 将来の oboe / RemoteIO 実装が追加時にログを失うことへ気付けるよう、必須メソッド
    /// としている。
