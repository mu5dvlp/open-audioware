...
- `sound`: `SoundData`(f32 ステレオ・インターリーブ PCM)、`SoundId`、`SoundStorage`
  ...
  絶対に使わない)。
- `wav`: 48kHz / 16bit PCM / モノラル・ステレオの wav を自前パーサでデコードする。
  モノは等パワー(`1/√2`)で両ch展開。48kHz 以外・非対応フォーマットは
  `WavError` の具体的なバリアントで返す(サンプルレート不一致のメッセージには
  M2 で rubato 導入予定である旨を含める)。`#[cfg(test)] pub mod golden` に
  ゴールデンテスト用の wav バイト列ビルダを置く(バイナリはコミットしない。§8)。
...
将来 `symphonia`(デコード)・`rubato`(リサンプル)を追加する(初期構築仕様 §7.1、M2)。
