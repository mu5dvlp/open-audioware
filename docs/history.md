...
| M3 実運用耐性 | **着手**(iOS の割り込み・バックグラウンド復帰・ルート変化〔OldDeviceUnavailable〕・アンダーラン検知テレメトリは実装済み。Android の AAudio disconnect 再オープンは設計調査が完了し、`Instance` の所有モデル再設計が要る2案〔案A/案B〕を報告して判断待ち。実装は未着手) |
