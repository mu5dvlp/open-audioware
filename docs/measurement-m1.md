...
- [ ] 見積もり(§8.7)の前提を実測で裏取りする — 音声コールバックが実際に受け取った
      フレーム数をアトミック変数経由で読み出して I/O バッファ長と突き合わせる
...
- `IOBufferDuration` / `outputLatency` がいずれもちょうど 5.000ms と出ているのは
  ...
  (`setActive(true)` の直後に読んでいる)。より確実にするなら、音声コールバックが
  実際に受け取ったフレーム数を(アトミック変数経由で、リアルタイム安全性を保ったまま)
  ゲームスレッドから読み出して突き合わせる
