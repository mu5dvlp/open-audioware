...
- [ ] 起動ログ `[mw-backend] AVAudioSession configured: ...` を記録する(第2回では取り損ねた。§8.1)
...
- **取り損ねた記録**: 起動ログの `[mw-backend] AVAudioSession configured: ...`。
  OS が実際に採用した I/O バッファ長・出力レイテンシが分からないままなので、
  **次回は必ずこの1行を残すこと**(§8.6-1 の見積もりに要る)
