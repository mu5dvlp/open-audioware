...
- **ライセンスは MIT-0** —— 著作権表示すら不要。商用利用も無制限
  ([詳細](#ライセンス))
- **Unity からは UPM パッケージとして使う**(C# の薄いラッパ経由)
...
**リアルタイム安全性は自動検証しています** —— 音声コールバック内でのアロケーションゼロを
テストで固定しており、オフラインレンダリングによる出力の回帰テストも揃っています
(`make test` で 300 件超)。
...
- **UPM のパッケージ名は未確定**です(公開レジストリには出していません)
- 実機確認は開発者の手元の iPhone / Android 各1台で行っています ——
  **機種ごとの網羅的な検証はしていません**

そのうえで、**設計判断の理由はすべて文書に残してあります**:
...
📌 [`CLAUDE.md`](./CLAUDE.md) はこのリポジトリで作業するとき(AI エージェント含む)の
運用ルールです。**使うだけなら読む必要はありません。**
...
📌 **使うだけなら不要です** —— ビルド済みバイナリを含む UPM パッケージを取り込む手順は
[`docs/integration.md`](./docs/integration.md) にあります。以下は**ミドルウェア自体を
ビルド・改造する人**向けです。
...
各クレートの設計・不変条件(特に**リアルタイム安全性規約** —— 音声スレッドで何をしては
いけないか)は `crates/<name>/CLAUDE.md` にあります。**改造するなら必ず目を通してください。**

## 貢献について

Issue / PR を歓迎します。⚠️ ただし**ソロで開発しているため反応は遅いことがあります**。

- 変更前に `make lint` と `make test` が通ることを確認してください
- **音声コールバック内でアロケーションする変更は受け付けられません**
  (リアルタイム安全性規約。テストが機械的に弾きます)
- 依存を足したら `make third-party-licenses` を実行してください(`make lint` が鮮度を検査します)
