...
- 実装方針・決定事項・フェーズ計画の唯一の正は **テンプレート仕様「初期構築仕様」**
  (`init.md`。詳細はリポジトリ直下のドキュメントを参照)
- 運用ルールの索引は [`CLAUDE.md`](./CLAUDE.md) を参照

現在のフェーズ: **M0(基盤)**。git workspace / 3クレート構成 / FFI スケルトン /
csbindgen によるバインディング生成 / UPM パッケージ骨格 / 統合検証用 Unity プロジェクト /
CI(4ターゲットのビルド検証)までが揃っている。
